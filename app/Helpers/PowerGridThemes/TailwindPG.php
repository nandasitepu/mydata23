...
class TailwindPG extends Tailwind
{
    public string $name = 'tailwind';

    public function table(): Table
    {
        return Theme::table('min-w-full px-10 table-fixed')
        ->div('my-3 oveflow-auto table-fixed')
        ->thead('shadow-sm bg-slate-100 border border-slate-200 text-sm')
        ->thAction('!font-bold')
        ->tdAction('')
        ->tr('bg-red-200 ')
        ->trFilters('bg-white shadow-sm')
        ->th('font-semibold p-2 font-semibold text-slate-700 tracking-wider whitespace-nowrap ')
        ->tbody('text-slate-800 text-sm whitespace-nowrap')
        ->trBody('border border-slate-100 hover:bg-slate-50')
        ->tdBody('px-3')
        ->tdBodyTotalColumns('px-3 whitespace-nowrap text-sm text-slate-600 space-y-2');
    }
